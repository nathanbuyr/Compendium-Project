...
export class SavedMonstersService {
  ...
  removeMonster(monster: any): void {
    const index = this.savedMonsters.findIndex(m => m.id === monster.id) /*findIndex checks each element in savedMonsters to see if the monster that was passed through is saved*/;
